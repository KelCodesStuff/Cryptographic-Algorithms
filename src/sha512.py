...
# Right rotate function for 64-bit integers
...
# Padding function for SHA-512
def sha512_pad(message):
    length = struct.pack('>Q', len(message) * 8)
    message += b'\x80'
    message += b'\x00' * ((112 - len(message) % 128) % 128)
    message += b'\x00' * 8 + length
    return message
...
def sha512(message):
    ...
    message = sha512_pad(message)

    for i in range(0, len(message), 128):
        chunk = message[i:i+128]
        hash_state = sha512_compress(chunk, hash_state)
...
def example2():
    ...
    print(f"Example 2: This is the SHA-512 Hashing Algorithm = {sha512(message)}")

def example3():
    message = b"My name is KelCodes and I wrote this program"
    print(f"Example 3: My name is KelCodes and I wrote this program = {sha512(message)}")
